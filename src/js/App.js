const App = (function() {
      ...
      $.each(results, function (i) {
        if (results[i].type.value === types.person) {
          $peopleList.append(Template.author.render({
            shortLink: results[i].link.value.slice(41),
            link: results[i].link.value,
  ...
  function renderAuthor(json) {
    ...
    $.each(results, function (i) {
      if (results[i].type.value === types.paper) {
        $paperList.append(Template.paper.render({
          shortLink: results[i].paper.value.slice(41),
          link: results[i].paper.value,
          name: results[i].title.value,
          year: results[i].year.value
        }));
      } else if (results[i].type.value === types.person) {
        $peopleList.append(Template.author.render({
          shortLink: results[i].knows.value.slice(41),
          link: results[i].knows.value,
          name: results[i].coname.value
        }));
      } else if (results[i].type.value === types.affiliation) {
        // Map.setAffiliation(results[i]);
      }
  ...
  function renderPaper(json) {
    ...
    $.each(results, function (i) {
      if (i > 0) {
        $peopleList.append(Template.author.render({
          shortLink: results[i].coauthor.value.slice(41),
          link: results[i].coauthor.value,
          name: results[i].name.value
        }));
      }
    });
